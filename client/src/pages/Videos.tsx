...
const Videos = () => {
  const { response: youtubeVideos = [], isLoading }: YoutubeVideoData = useAxios({
    url: '/api/youtubevideos'
  });

  const [page, setPage] = useState(0);
  const [visibleVideos, setVisibleVideos] = useState<YoutubeVideoInterface[]>(youtubeVideos);
  const videosPerPage = 36;
  const totalPages = calculateTotalPages(youtubeVideos.length, videosPerPage);
  const ref = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (!youtubeVideos || !youtubeVideos.length) return setVisibleVideos([]);

    const pagedVideos = youtubeVideos.slice(page * videosPerPage, (page + 1) * videosPerPage);

    setVisibleVideos(pagedVideos);
  }, [page, youtubeVideos, videosPerPage]);
